...
class WOSettings extends WOOPtions {
	 ...
	 * @param bool   $call_settings_errors Whether or not to call the settings_errors() function.
	 *
	 * @return void
	 */
	public function settings_page( $admin_title, $admin_url, $settings, $call_settings_errors = true ) {
		$this->start();
		$this->title( $admin_title );
		$this->form_start();

		if ( $call_settings_errors ) {
			settings_errors();
		}
